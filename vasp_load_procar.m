...
%         plot(1:Nkpts,bs',symb{mod(floor((var_type-1)/length(c(:,1))),length(symb))+1},...
%             'markeredgecolor',c(mod(var_type-1,length(c(:,1)))+1,:));

        plot(1:Nkpts,bs',symb{mod((var_type-1),3)+1},...
             'markeredgecolor',c(floor((var_type-1)/3)+1,:),'MarkerSize',5);
    end
    ...
    [l,icons,plots,legend_text]=legend(a_names);
...
%     
%    
%     [l,icons,plots,legend_text]=legend(legendstring);
%     %f = get(l,'children');
%     %set(f(1),'markersize',5);
%     for i = 1:length(icons)
%         icons(i).LineWidth = 5
%     end
    [lh,lo] =findall(gcf,'tag','legend');
